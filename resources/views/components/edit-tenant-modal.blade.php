...
>
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] sm:max-h-[80vh] flex flex-col overflow-hidden transform transition-all duration-300 scale-100">
    ...
    <div class="flex items-center justify-between border-b border-slate-200 px-4 sm:px-6 py-3 sm:py-4">
      <h2 class="text-base sm:text-lg font-semibold text-slate-800">Edit Tenant</h2>
      <button type="button" onclick="hideEditTenant()" class="text-slate-500 hover:text-slate-700 text-xl">✕</button>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 flex-1 overflow-hidden">

      {{-- Form --}}
      <form method="POST" action="{{ admin_url('admin-post.php') }}" class="p-4 sm:p-6 space-y-4 sm:space-y-5 h-full overflow-y-auto overscroll-contain min-h-0" id="editTenantForm">
        {{-- Hidden inputs --}}
        ...
        {{-- Lease Start Date --}}
        <div>
          <label for="edit_lease_start_date" class="block text-sm font-medium text-slate-700 mb-1">Lease Start Date</label>
          <input type="date" id="edit_lease_start_date" name="lease_start_date"
                class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 focus:outline-none">
        </div>

        {{-- Lease End Date --}}
        <div>
          <label for="edit_lease_end_date" class="block text-sm font-medium text-slate-700 mb-1">Lease End Date</label>
          <input type="date" id="edit_lease_end_date" name="lease_end_date"
                class="w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 focus:outline-none">
        </div>
